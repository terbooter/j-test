...
export const ListHandler = async (
  ...
  if (request.query.parent === undefined) {
    blocks = await blockService.getByWorkspace(request.query.cursor);
  } else {
    const parent = await blockService.getByLink(request.query.parent);
    if (parent === null) {
      return reply
        .status(200)
        .send({ data: [], meta: { nextCursor: null, prevCursor: null } });
    }
    blocks = await blockService.getByParent(parent.id, request.query.cursor);
  }
